...
inline void solve()
{
    int n, m;
    cin >> n >> m;
    vector<vector<int>> arr(n + 1, vector<int>(m + 1)), dp(n + 1, vector<int>(m + 1)), dp1(n + 1, vector<int>(m + 1));
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= m; j++)
            cin >> arr[i][j];
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        dp1[i][1] = arr[i][1] + dp1[i - 1][1];
    }
    for (int i = 1; i <= m; i++)
    {
        dp[1][i] = arr[1][i] + dp[1][i - 1];
        dp1[1][i] = arr[1][i] + dp1[1][i - 1];
    }
    for (int i = 2; i <= n; i++)
    {
        for (int j = 2; j <= m; j++)
    ...
    bool zeroIsinMiddle = dp[n][m] <= 0 && dp1[n][m] >= 0;
    if (zeroIsinMiddle && (n + m) & 1)
        cout << "YES" << endl;
    else
        cout << "NO" << endl;
}
