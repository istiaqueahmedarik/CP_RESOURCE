...
int checkSum(vector<int> &arr, int N,
             int sm1, int sm2,
             int sm3, int j)
{
    if (dp[sm1][sm2][sm3][j])
        return dp[sm1][sm2][sm3][j];
    if (j == N)
    {
        if (sm1 == sm2 && sm2 == sm3)
            return dp[sm1][sm2][sm3][j] = 1;
        else
            return dp[sm1][sm2][sm3][j] = 2;
    }

    else
    {
        int l = checkSum(arr, N,
                         sm1 + arr[j],
                         sm2, sm3, j + 1);

        int m = checkSum(arr, N, sm1,
                         sm2 + arr[j],
                         sm3, j + 1);

        int r = checkSum(arr, N, sm1, sm2,
                         sm3 + arr[j], j + 1);

        return dp[sm1][sm2][sm3][j] = max(max(l, m), r);
    }
}
