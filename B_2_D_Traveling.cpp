...
inline void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        cin >> x >> y;
        if (i < k)
            majors.push_back({x, y});
        alls.push_back({x, y});
    }
    pair<int, int> src = alls[a - 1], dest = alls[b - 1];
    int mnCharge = manHattan(src, dest);
    int mnFromSrc = LLONG_MAX;
    for (auto i : majors)
    {
        mnFromSrc = min(mnFromSrc, manHattan(src, i));
    }
    int mnFromDest = LLONG_MAX;
    for (auto i : majors)
    {
        mnFromDest = min(mnFromDest, manHattan(dest, i));
    }
    if (mnFromDest != LLONG_MAX)
    {
        mnCharge = min(mnCharge, mnFromSrc + mnFromDest);
    }
    cout << mnCharge << endl;
}
