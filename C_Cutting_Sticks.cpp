...
int ans(int left, int right, vector<int> &cuts, vector<vector<int>> &dp)
{
    debug(left, right);
    if (left > right)
        return 0;
    ...
    for (int i = 0; i < cuts.size(); i++)
    {
        ...
        {
            ret = min(ret, ans(left, cuts[i], cuts, dp) + ans(cuts[i], right, cuts, dp) + (right - left));
        }
    }
    if (ret == LLONG_MAX)
...
inline void solve()
{
    while (1)
    {
        ...
        cout << "The minimum cutting is " << ans(0, len, cuts, dp) << "." << endl;
    }
}
