...
inline void solve()
{
    ...
    int q;
    cin >> q;
    unordered_map<int, vector<int>> allList;
    while (q--)
    {
        int x;
        cin >> x;
        x--;
        vector<int> temp;
        int idx = lower_bound(all(temp), x) - temp.begin();
        temp.push_back(max(x, l[idx] + r[idx] - x));
        allList[idx] = temp;
    }
    for (auto i : allList)
    {
