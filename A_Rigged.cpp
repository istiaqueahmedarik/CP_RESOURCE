...
inline void solve()
{
    ...
    // tar strength er majhe tar caite beshi jader endurance tader strength er caite beshi
    vector<pair<int, int>> allPlayers;
    for (int i = 0; i < n - 1; i++)
    {
        int a, b;
        cin >> a >> b;
        allPlayers.push_back({a, b});
    }
    vector<pair<int, int>> beshi;
    for (auto i : allPlayers)
    {
        if (i.second >= me.second)
        {
            beshi.push_back(i);
        }
    }
    sort(rall(beshi));
    if (beshi.size() && beshi[0].first + 1 <= me.first)
    {
        cout << beshi[0].first + 1 << endl;
    }
    else if (!beshi.size())
    {
        cout << me.first << endl;
