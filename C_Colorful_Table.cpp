...
inline void solve()
{
    ...
    for (int i = 1; i <= n; i++)
        cin >> arr[i];
    unordered_map<int, int> leftest, rightest;
    for (int i = 1; i <= n; i++)
    {
        if (!leftest[arr[i]])
            leftest[arr[i]] = i;
    ...
    for (int i = 0; i < n - 1; i++)
    {
        leftest[arr[i]] < leftest[arr[i + 1]] ? leftest[arr[i + 1]] = leftest[arr[i]] : 0;
        rightest[arr[i]] > rightest[arr[i + 1]] ? rightest[arr[i + 1]] = rightest[arr[i]] : 0;
    }
    for (int i = 1; i <= k; i++)
    {
        if (!leftest[i])
        ...
        {
            int len = rightest[i] - leftest[i] + 1;
            cout << 2 * len << " ";
        }
    }
    cout << endl;
