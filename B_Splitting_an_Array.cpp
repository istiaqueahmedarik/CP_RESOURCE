...
vector<int> arr, pre;
int n, k;
bool check(vector<int> pre, int ans)
{
    int cnt = 0, first = pre[1], diff = 0, preInd = 1;
    for (int i = 2; i <= n; i++)
    {
        if (pre[i] - first > ans)
        {
    ...
    debug(ans, cnt, diff, n - diff, k - cnt);
    return cnt <= k - 1 && cnt >= 1;
}
inline void solve()
{
    cin >> n >> k;
    ...
    int l = *max_element(all(arr)), r = pre[n], ans = 0;
    while (l <= r)
    {
        int mid = (l + r) >> 1;
        if (!check(pre, mid))
