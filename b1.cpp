...
void findFactors(int x, vector<int> &currentFactors, vector<vector<int>> &allFactors, int start)
{
    if (x == 1)
    {
    ...
    for (int i = start; i <= x; i++)
    {
        if (x % i == 0)
        {
            ...
            findFactors(x / i, currentFactors, allFactors, i);
            currentFactors.pop_back();
        }
    }
}

vector<vector<int>> getAllFactors(int x)
{
    vector<vector<int>> allFactors;
    vector<int> currentFactors;
    findFactors(x, currentFactors, allFactors, 2);
    return allFactors;
}
inline void solve()
{
    int n;
    cin >> n;
    vector<vector<int>> factors = getAllFactors(n);
    vector<int> ans;
    for (auto &i : factors)
    {
        int sm = 0;
