...
inline void solve()
{
    ...
    if (n & 1)
    {
        float ans = (float)INT_MIN;
        for (int i = 0; i < n; i++)
        {
            int prev = i - 0;
            int nxt = n - (i + 3);
            if (prev & 1 ^ 1 && nxt & 1 ^ 1 && nxt >= 0)
            {
                vector<float> b;
                for (int j = 0; j < n;)
                {
                    if (j == i)
                    {
                        b.push_back((float)(a[j] + a[j + 2]) / 2.0);
                        j += 3;
                    }
                    else
                    {
                        b.push_back((float)(a[j] + a[j + 1]) / 2.0);
                        j += 2;
                    }
                }
                float mx = *max_element(all(b));
                float mn = *min_element(all(b));
                ans = max(ans, mx - mn);
            }
        }
        cout << ans << endl;
    ...
    else
    {
        ...
        float mn = *min_element(all(b));
        float mx = *max_element(all(b));
        cout << fixed << setprecision(3) << mx - mn << endl;
    }
}
