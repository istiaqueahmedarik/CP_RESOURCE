...
bool isSet(int x, int i) { return ((x >> i) & 1); }
bool isXorSet(int x, int i) { return (((x >> i) ^ 1) & 1); }
bool isInRange(int x, int i, int r)
{
    return (x | (1 << i)) <= r;
}
inline void solve()
{
    ...
    while (q--)
    {
        ...
        while (curr >= 0)
        {
            if (isInRange(mx, curr, p) && isSet(jodiOne, curr) && isXorSet(jodiZero, curr))
                mx |= (1 << curr);
            curr--;
        }
        cout << mx << endl;
