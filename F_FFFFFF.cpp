...
inline void solve()
{
    ...
    for (int i = 1; i <= n; i++)
        cin >> a[i];
    // a_i + j = i*k + j*k -> a_i = i*k + j*k - j -> j = (a_i - i*k)/(k-1);
    vector<pair<int, int>> sz;
    map<int, map<int, bool>> index;
    for (int i = 1; i <= n; i++)
    {
        for (int k = 2; a[i] - k * i >= 0; k++)
        {
            int temp = (a[i] - i * k) / (k - 1);

            if ((a[i] - i * k) % (k - 1) == 0 && temp >= 1 && temp <= n && temp != i)
            {
                int gcd = __gcd(a[i] + temp, a[temp] + i);
                if (gcd == i + temp && !index[i][temp])
                {
                    sz.push_back({i, temp});
                    index[i][temp] = true;
                    index[temp][i] = true;
