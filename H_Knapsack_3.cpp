...
vector<pair<int, int>> coins(3);
const int MX = 5100 * 5100;
int dp[MX];
inline int mx(int n)
{
    if (dp[n] != -1)
    ...
    for (int i = 0; i < 3; i++)
    {
        if (n - coins[i].first >= 0)
        {
            ans = max(ans, coins[i].second + mx(n - coins[i].first));
        }
    }
    return dp[n] = ans;
}
...
inline void solve()
{
    ...
    memset(dp, -1, sizeof(dp));
    for (int i = 0; i < 3; i++)
        swap(coins[i].first, coins[i].second);
    cout << mx(newCoin) << endl;
}
