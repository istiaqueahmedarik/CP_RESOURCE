...
int ans = 0;
void IsPos(vector<int> &ar, int curSm, int targetSm, int curInd, int n)
{
    if (curInd < 0)
        return;

    if (curSm + ar[curInd] == targetSm)
        ans++;
    if (curSm + ar[curInd] < targetSm)
    {

        IsPos(ar, curSm + ar[curInd], targetSm, curInd - 1, n);
    }
    IsPos(ar, curSm, targetSm, curInd - 1, n);
}
