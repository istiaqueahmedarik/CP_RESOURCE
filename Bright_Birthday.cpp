...
vector<string> col = {"blue", "green", "yellow", "red", "purple", "orange", "pink", "grey", "cyan", "brown", "ash", "silver", "gold", "white", "black"};
map<int, map<vector<int>, int>> mp;
int ans(int curInd, vector<int> b)
{
    if (curInd < 0)
        return 0;
    if (mp[curInd][b])
        return mp[curInd][b];
    if (count(all(b), 0) == b.size())
    {
        return 0;
    }
    bool canChoose = true;
    string cur = col[curInd];
    vector<int> cpy = b;
    ...
    int take = LLONG_MIN;
    if (canChoose)
        take = ans(curInd - 1, cpy) + 1;
    int leave = ans(curInd - 1, b);
    return mp[curInd][b] = max(take, leave);
}
