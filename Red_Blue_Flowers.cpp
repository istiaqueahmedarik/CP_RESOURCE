...
int n;
int final = 0;
int ans(vector<vector<int>> &dp, vector<int> &red, vector<int> &green, int index, int x, int y)
{
    if (index == n)
    {
        return min(x, y);
    ...
    {
        dp[index][x] = y;
        int p1 = ans(dp, red, green, index + 1, x + red[index], y);
        int p2 = ans(dp, red, green, index + 1, x, y + green[index]);
        return max(p1, p2);
    }
    return 0;
}
void solve()
{
    vector<vector<int>> dp(1000, vector<int>(21000, -1));
    cin >> n;
    ...
    for (int i = 0; i < n; i++)
        cin >> green[i];
    cout << ans(dp, red, green, 0, 0, 0) << endl;
}
