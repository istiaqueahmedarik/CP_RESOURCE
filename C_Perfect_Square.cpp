...
int isUnchangedAfterRotation(vector<vector<char>> &matrix)
{
    int c = 0;
    int n = matrix.size();
    for (int i = 0; i < matrix.size(); i++)
    {
        for (int j = 0; j < matrix.size(); j++)
        {
            ...
            {
                //  debug(matrix[i][j], matrix[n - 1 - j][i]);
                // cout << endl;

                char tmp = max(matrix[n - 1 - j][i], matrix[i][j]);
                if (tmp != matrix[n - 1 - j][i])
                    c += tmp - matrix[n - 1 - j][i];
                else if (tmp != matrix[i][j])
                    c += tmp - matrix[i][j];
                matrix[n - 1 - j][i] = tmp;
                matrix[i][j] = tmp;
            }
...
inline void solve()
{
    ...
    int cnt = isUnchangedAfterRotation(matrix);
    cout << cnt << endl;
}
