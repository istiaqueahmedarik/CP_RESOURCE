...
inline void solve()
{
    ...
    int c;
    cin >> c; // 1--> n
    function<bool(int, vector<int> &, int)> isSafe = [&](int u, vector<int> &col, int c)
    {
    ...
    function<void(vector<int> &, int, int, int)> dfs = [&](vector<int> &curColor, int rem, int u, int c)
    {
        debug(curColor);
        if (rem == 0)
        {
        ...
        for (auto j : adjList[u])
        {
            if (curColor[j] == 0)
            {
                for (int k = 1; k <= c; k++)
                {
                    if (!isSafe(j, curColor, k))
                        continue;
                    if (k == c)
                        continue;
                    curColor[j] = k;
                    dfs(curColor, rem - 1, j, k);
                    curColor[j] = 0;
                }
            }
        }
    ...
    for (int i = 1; i <= n; i++)
    {
        for (int j = 1; j <= c; j++)
        {
            vector<int> curColor(n + 1, 0);
            curColor[i] = j;
            dfs(curColor, n - 1, i, j);
        }
    }
}
